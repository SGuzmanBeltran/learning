...
class OnlineStore:
    ...
    def register_user(self, id, name, email, password, address, payment_info):
        ...
        self.notification_service.send_email(
            ...
            f"Dear {name}, thank you for registering!"
        )
    ...
    def create_order(self, user_id, items):
        ...
        for item in items:
            ...
            # Update stock
            self.update_product_stock(product["id"], product["stock_quantity"] - item["quantity"])
...
class Database:
    ...
    def save_report(self, report):
        print(f"Saving report to database for period: {report['start_date'].strftime('%Y-%m-%d')} to {report['end_date'].strftime('%Y-%m-%d')}")
        # In a real implementation, this would save to a database
        return True
...
if __name__ == "__main__":
    ...
    # Add products
    store.add_product(1, "Laptop", 999.99, "Electronics", "Powerful laptop for developers", 20)
    store.add_product(2, "Headphones", 149.99, "Electronics", "Noise-cancelling headphones", 30)
    ...
    store.register_user(
        ...
        {"type": "credit_card", "card_number": "1234567890123456", "expiry_date": "12/24", "cvv": "123"}
    )

    # Create an order
    store.create_order(1, [
